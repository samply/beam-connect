...
pub(crate) fn example_central(broker_id: &BrokerId) -> CentralMapping {
    ...
    let app1_id = AppId::new(&format!("connect1.{}",proxy23.to_string())).unwrap();
    let app2_id = AppId::new(&format!("connect2.{}",proxy23.to_string())).unwrap();
    let uk12_id = AppId::new(&format!("connect.{}",uk12_proxy.to_string())).unwrap();
    let map = [
